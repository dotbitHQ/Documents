...
提案在被确认之前必须等待 `ConfigCellProposal.proposal_min_confirm_interval` 个区块高度。
...
允许提案发起者回收无法通过的提案，从而回收 ProposalCell 占用的 CKB。提案在被回收之前必须等待 `ConfigCellProposal.proposal_min_recycle_interval` 个区块高度。
